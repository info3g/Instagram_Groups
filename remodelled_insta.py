...
def instagram(user_name,passwd,excel_data_df):
    ############################## LOGIN ########################################
...
def follow_on_submit(user_list):
...
if __name__ == '__main__':
    ...
    excel_data_df = pandas.read_excel('Agent_Player_List.xlsx')
    credentials_list = [[constants_df['Credentials_un'][0],constants_df['passwd'][0]],
                        [constants_df['Credentials_un'][1],constants_df['passwd'][1]],
                        [constants_df['Credentials_un'][2],constants_df['passwd'][2]]]

    index = random.randint(0,2)
    user_name = credentials_list[index][0]
    passwd = credentials_list[index][1]
    # print(f'{user_name}    {passwd}')                    
    # #1st ID
    # user_name = constants_df['Credentials_un'][0]
    # passwd = constants_df['passwd'][0]
    s = time.time()
    user_dict,excel_data_df = instagram(user_name,passwd,excel_data_df)
    f = time.time()
    print(f'time taken: {f-s} secs')
